...
class Voyage extends CI_Controller
{
    ...
    public function annulerTicket()
    {
        $data = array('statut'=>"Annulé");

        $id_passager = $this->input->post("id_passager");

        // récupération du transport du passager
        $transport = $this->VoyageModel->getPassagerTransport($id_passager);

        $tp = $transport * (-1);

        $data['tarif'] = $tp;

        if($this->VoyageModel->annulerTicket($id_passager,$data)){
            echo json_encode("1");
        }else{
            echo json_encode('0');
        }
